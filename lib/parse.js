...
function parseBoolFromString(string) {
  if (string === 'true') {
    ...
    //Luxon help us to parse a string in a Type `Date`, then, with
    // `isValid` property we can check if `string` is a real Date.
    var isValidDate = luxon.DateTime.fromISO(string).isValid;
    if (isValidDate) { return new Date(string);}
    return string;
  }
